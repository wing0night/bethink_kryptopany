...
export const useFileStatus = (fileId: number | null) => {
  ...
  const addFile = async (url: string, encryptedKey: string) => {
    if (!contractAddress || !walletAddress) {
      setError("Wallet not connected, contract address not set");
      return;
    }
    ...
    try {
      ...
      const tx = await contract.addFile(url, encryptedKey);
      await tx.wait();
      setError(null);
    } catch (err: any) {
      console.error("Error adding file:", err);
      setError(`Failed to add file: ${err.message || 'Unknown error'}`);
    }
  }
  

  return { isFinalized, reward, isClaimable, isClaiming, claimReward, addFile, error };
};
